...
class ChatList extends StatelessWidget {
  final String roleFilter; // sell, donate, exchange, request
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final userChats = allChats.where((doc) {
          ...
          // Include only if current user is a participant AND itemType matches the tab (roleFilter)
          return participants != null &&
              participants.containsKey(currentUserId) &&
              itemType == roleFilter.toLowerCase();
        }).toList();

        if (userChats.isEmpty) {
          return const NoMessageWidget(); // Show no messages widget
        }
